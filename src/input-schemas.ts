...
export const randomChoiceInputSchema = z
  .object({
  ...
  .superRefine(({ choices, weights, count, with_replacement }, ctx) => {
    ...
    if (count > choices.length) {
      ctx.addIssue({
        code: "custom",
        message: "count must not exceed the number of choices when sampling without replacement",
        path: ["count"],
      });
    }
    ...
    if (
      ...
      ctx.addIssue({
        code: "custom",
        message: "count must not exceed the number of positive weights when sampling without replacement",
        path: ["count"],
      });
    }
  });
