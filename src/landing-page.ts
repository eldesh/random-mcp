...
import { landingPageContent, type Locale, type ToolContent } from "./landing-page-content.ts";
...
export function preferredLocale(acceptLanguage: string | null): Locale {
  ...
  const languages = acceptLanguage
    ...
    .map((entry, index) => {
      ...
      const parsedQuality = qualityParameter === undefined ? 1 : Number.parseFloat(qualityParameter.trim().slice(2));
